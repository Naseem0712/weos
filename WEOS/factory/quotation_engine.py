...
from WEOS.factory.job_types import GlassPane, LineItem, QuotationResult, WeightBreakdown
...
def compute_quotation(
    ...
    *,
    ...
    colour_pct = float(quote_rules.get("_colourSurchargePercent", 0) or 0)
    if colour_pct:
        colour_amt = round(subtotal * (colour_pct / 100.0), 2)
        lines.append(
            {
                "category": "colour",
                "description": f"Colour surcharge ({quote_rules.get('_colour', '')} {colour_pct}%)",
                "qty": 1,
                "unit": "pct",
                "rate": colour_pct,
                "amount": colour_amt,
        ...
        subtotal = round(subtotal + colour_amt, 2)
