...
def build_customer_pdf_bytes(payload: Mapping[str, Any]) -> bytes:
    ...
    When ``pdfFailClosed`` is set (customer Download/Print), never degrade to a
    demo-terms reportlab layout or a minimal stub PDF — raise instead.
    """
    fail_closed = bool(payload.get("pdfFailClosed"))
    try:
        from WEOS.factory.template_pdf import render_template_pdf

        pdf = render_template_pdf(payload, kind="customer")
        if fail_closed:
            from WEOS.factory.pdf_preflight import assert_not_minimal_pdf
        ...
        if isinstance(exc, PdfCompletenessError):
            raise
        if fail_closed:
            _log.exception("customer template PDF failed (fail-closed)")
            raise PdfCompletenessError(
                "Customer PDF render failed",
                errors=[str(exc)],
            ) from exc
...
def _customer_reportlab(payload: Mapping[str, Any]) -> bytes:
    ...
    set_font(c, 8)
    # Never inject demo/default terms. Print quote or company terms only.
    terms = str(payload.get("terms") or "").strip()
    if not terms:
        branding = payload.get("branding") if isinstance(payload.get("branding"), Mapping) else {}
        terms = str((branding or {}).get("terms") or "").strip()
    if terms:
        for i, row in enumerate(terms.splitlines()[:6]):
            c.drawString(40, y, row[:95])
            y -= 11
            if y < 60:
                break
    else:
        c.drawString(40, y, "Terms: (none supplied — see company quotation terms separately).")
        y -= 12
